...
async function handleOrdenarIntent(event, sessionAttributes, userInput) {
    ...
    if (tieneOrdenActiva) {
        console.log("SI ha encontrado alguna orden activa")
            ...
            return {
                sessionState: {
                    ...
                    sessionAttributes: {
                        ...sessionAttributes,
                        requiresHumanIntervention: true,
                        reason: "El cliente realizó multiples intentos para ordenar a pesar de que ya tiene una orden activa. Contactar para validar si tiene dificultades"
                    }
                },
                messages: [
                    {
        ...
        return {
            ...
            messages: [
                {
                    contentType: "PlainText",
                    content: "Lo siento, ya tienes una orden activa."
                },
                {
                    contentType: "PlainText",
                    content: "Solo puedes tener una orden activa a la vez."
                }
            ]
        };
    }
    ...
    try {
        ...
        if (!sessionAttributes.elementoActual && sessionAttributes.elementosPendientes) {
            ...
            if (elementoActual.atributos && elementoActual.atributos.length > 0) {
                ...
                const mensajeElemento = elementoActual.totalUnidades > 1 ?
                    `Para tu ${elementoActual.nombre} ${elementoActual.unidadNumero} de ${elementoActual.totalUnidades}:` :
                    `Para tu ${elementoActual.nombre}:`;
                ...
                if (atributosRestantes.length > 0) {
                    ...
                    if (elementosArray.length > 0) {
                        ...
                        const mensajeElementoSiguiente = siguienteElemento.totalUnidades > 1 ?
                            `Para tu ${siguienteElemento.nombre} ${siguienteElemento.unidadNumero} de ${siguienteElemento.totalUnidades}:` :
                            `Para tu ${siguienteElemento.nombre}:`;
        ...
        delete sessionAttributes.totalElementos;
...
async function handleAgregarAOrdenIntent(event, sessionAttributes, intentInfo, userInput) {
    ...
    console.log("Condición quese usara en el IF:", sessionAttributes.ordenParaProcesar && !(event.sessionState.intent.slots.atributosCapturados));
    ...
    if (!sessionAttributes.elementosPendientes) {
        ...
        // Expandir elementos según cantidad
        const elementosExpandidos = [];
        categoriaInfo.elementos.forEach(elemento => {
            for (let i = 0; i < elemento.cantidad; i++) {
                elementosExpandidos.push({
                    ...elemento,
                    cantidad: 1,
    ...
    if (sessionAttributes.atributosCapturados < sessionAttributes.numeroTotalAtributos) {
        ...
        sessionAttributes.inputAcumulado += ` ${elementoActual.nombre}_${sessionAttributes.atributoActualNombre}:${slotAtributo}`;
        nuevoInput = sessionAttributes.inputAcumulado;
    ...
    try {
        ...
        mensajes.push({
            contentType: "PlainText",
            content: "Listo, he agregado tu pedido a la orden."
        })
...
export async function getMenu() {
    ...
    try {
        ...
        console.log("Respuesta completa de la hoja 'MENU':", menu.data);
